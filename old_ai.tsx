...
/** AI opponent cars ΓÇö -Z is forward, same as player */
export function AIOpponents({ playerSpeed, aiSpeedMult = 1, onPositionUpdate, onDetailedUpdate, playerDistRef, track, aiKnockbackRef, randomFn }: Props) {
    ...
    useFrame((_, dt) => {
        ...
        cars.forEach((c, i) => {
            // Rubberbanding ΓÇö based on relative z position
            const relZ = c.z; // c.z < 0 = ahead of player (who is at z=0)

            let target = c.maxSpeed;
            if (relZ < -50) target = pSpeed * 0.7 * aiSpeedMult;       // Too far ahead ΓåÆ slow
            else if (relZ < -25) target = pSpeed * 0.85 * aiSpeedMult;
            else if (relZ > 40) target = pSpeed * 1.4 * aiSpeedMult;  // Too far behind ΓåÆ speed up
